...
public class MainController {
    ...
    @GetMapping(value = "/searchRestaurantPage")
    public ModelAndView searchRestaurantPage()
    {
        ModelAndView mw = new ModelAndView("index");
        List<Restaurants> restaurants = restaurantRepository.findAll();
        List<Category> categories = categoryRepository.findAll();
        mw.setViewName("searchRestaurantPage");
        mw.addObject("category", categories);
        mw.addObject("restaurant", restaurants);
        return mw;
    }
    @PostMapping(value = "/searchRestaurantByName")
    public ModelAndView searchRestaurantByName(
            @RequestParam(name="restaurant_id")Long restaurant_id)
    {
        ModelAndView mw = new ModelAndView();
        Restaurants restaurant = restaurantRepository.getOne(restaurant_id);
        mw.setViewName("showSearchedRestaurants");
        mw.addObject("restaurant", restaurant);
        return mw;
    }

    @PostMapping(value = "/searchRestaurantByCategory")
    public ModelAndView searchRestaurantByCategory(
            @RequestParam(name="category_id")Long category_id)
    {
        ModelAndView mw = new ModelAndView();
        List <Restaurants> restaurants = restaurantRepository.findAll();
        Category category = categoryRepository.getOne(category_id);
        Iterator <Restaurants> iter = restaurants.iterator();
        while(iter.hasNext()){
            Restaurants element = iter.next();
            if (element.getCategory().id != category.id) {
                iter.remove();
            }
        }
        mw.setViewName("showSearchedRestaurants");
    ...
    @GetMapping(path = "/editCategory/{category_id}")
    @PreAuthorize("hasAnyRole('ROLE_ADMIN')")
    public String editCategory(ModelMap model, @PathVariable(name = "category_id") Long category_id){
        Category category = categoryRepository.getOne(category_id);
